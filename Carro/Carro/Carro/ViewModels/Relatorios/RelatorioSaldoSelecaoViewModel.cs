...
{
    public class RelatorioSaldoSelecaoViewModel : BaseViewModel
    {
        public RelatorioSaldoSelecaoViewModel(INavigation navigation) : base(navigation)
        {
        ...
        async Task ExecuteRelSaldoCommand()
        {
            if (!IsBusy)
            {
                ...
                IsBusy = true;
                await Navigation.PushAsync(new RelatorioSaldoResultadoPage(data));
                IsBusy = false;
            }
        }
        ...
        async Task ExecuteHelpCommand()
        {
            ...
            {
                IsBusy = true;
                await Navigation.PushAsync(new RelatorioSaldoSelecaoHelpPage());
                IsBusy = false;
            }
        }
    }
